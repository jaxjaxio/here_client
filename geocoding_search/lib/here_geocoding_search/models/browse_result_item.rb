  ...
  class BrowseResultItem
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `HereGeocodingSearch::BrowseResultItem` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `HereGeocodingSearch::BrowseResultItem`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [title, id, political_view, result_type, house_number_type, address_block_type, locality_type, administrative_area_type, house_number_fallback, address, position, access, distance, excursion_distance, map_view, categories, chains, references, food_types, contacts, opening_hours, time_zone, extended, phonemes, street_info].hash
    end
